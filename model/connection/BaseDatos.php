...
class BaseDatos extends PDO {
  ...
  public function Ejecutar($sql) //entra consulta por parametro
  {
    $this->setError("");
    $this->setSQL($sql); //carga la consulta en ebj DB
  ...
  private function EjecutarInsert($sql) {
    ...
    if (!$resultado) {
      ...
      $id =  $this->lastInsertId(); //retorna id de insersion SI ES AUTOINCREMENT EN LA BD
      if ($id == 0) {
        $id = -1;
      }
    }
  ...
  /**
   * Devuelve un registro retornado por la ejecucion de una consulta select
  ...
  public function Registro() {
    ...
    if ($indiceActual >= 0) {
      $filas = $this->getResultado(); // [fr] aca el arreglo de la consulta select
